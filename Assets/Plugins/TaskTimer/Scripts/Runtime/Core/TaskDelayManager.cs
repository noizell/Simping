...
namespace NPP.TaskTimers
{
    public class TaskDelayManager : MonoBehaviour
    {
        ...
        public TaskDelay CreateConditionalTask(float delay, System.Func<bool> condition, System.Action onConditionTrue, System.Action<int> onEachInterval, bool immediateRun)
        {
            _conditionalCachedTask = new ConditionalTaskDelay(delay, condition, onConditionTrue, onEachInterval, this);
            if (immediateRun)
        ...
        public TaskDelay CreateTask(float delay, System.Action onDoneWaitDelay, System.Action<int> onInterval, int interval, bool immediateRun, bool loop)
        {
            _cachedTask = new TaskDelay(delay, onDoneWaitDelay, interval, onInterval, loop, this);
            if (immediateRun)
                Enqueue(_cachedTask);
